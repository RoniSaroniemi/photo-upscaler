...
<!--
  ...
  =========================
  IMPORTANT: You are a SUPERVISOR. Your job is to DELEGATE work to the executor,
  not to implement it yourself. You brief the executor, monitor its progress,
  verify its output, and handle completion (commit, PR, cleanup).

  You should NOT:
  - Write code yourself (the executor writes code)
  - Edit project files directly (the executor does this)
  - Run builds or tests yourself unless verifying executor output

  You SHOULD:
  - Send the brief to the executor via tmux send-keys
  - Monitor the executor's progress via tmux capture-pane
  - Verify the executor's output (read files, run /verify, check tests)
  - Commit, push, and create the PR when work is complete
...
<!--
  When writing a supervisor brief, include the current lifecycle stage.
  This helps the supervisor understand WHY certain things are out of scope.
-->

**Current lifecycle stage:** [POC / Architecture / Alpha / Beta / Production]

**Allowed at this stage:** [copy from .cpo/lifecycle.md for the current stage]

**NOT allowed at this stage:** [copy from .cpo/lifecycle.md for the current stage]

**Scope lock:** This brief covers ONLY work appropriate for the current lifecycle stage.
If during execution you discover work that belongs to a later stage, note it in your
completion report but do NOT implement it. The CPO manages stage transitions.
