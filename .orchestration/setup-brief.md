...
## Step 5.4: Initialize Project Lifecycle

```bash
cp .cpo/templates/lifecycle.md .cpo/lifecycle.md
```

Then edit `.cpo/lifecycle.md`:
- Set `**Stage entered:**` to today's date
- In the POC checklist, fill in the "Core value proposition" item with the one-sentence description from Step 1
- Based on the project brief, identify 1-3 biggest unknowns and note them in the POC checklist's "biggest unknowns/risks" item
- Fill in the Project Pillars table with the pillars relevant to this project (based on tech stack and project type from Step 1)
- Leave the rest for the CPO to complete during its first cycle

The lifecycle tracks which stage the project is in (POC → Architecture → Alpha → Beta → Production) and prevents the CPO from skipping stages or building features too early.
...
```bash
tmux -L $PROJECT_SLUG send-keys -t cpo "Read CLAUDE.md for your project identity and instructions. Read .cpo/cpo-routine.md for your operating procedures. You are the CPO for [PROJECT_NAME], operating as [DISPLAY_TITLE]. Your tmux server is '$PROJECT_SLUG'. Use tmux -L $PROJECT_SLUG for all tmux commands. Start your cron loops now: first run /loop 30m with the prompt from .cpo/checks/cron-prompts.md (30-Minute Quick Check section). Then read .cpo/vision.md and .cpo/roadmap.md to understand the project. IMPORTANT: Read .cpo/lifecycle.md for the current project stage — before creating any briefs, verify the work is appropriate for the current stage and its checklist. When ready, report your status." Enter
sleep 3
tmux -L $PROJECT_SLUG send-keys -t cpo Enter
```
